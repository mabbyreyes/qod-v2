  ...
  public OAuth2Authentication loadAuthentication(String token)
    ...
    if (idToken != null) {
      ...
      String oauthKey = payload
          .getSubject(); // TODO Get any additional info from payload for current user.
      ...
      Authentication base = new UsernamePasswordAuthenticationToken(oauthKey, token,
          grants); // TODO User user object retrieved from user service, instead of oauthKey.
      OAuth2Request request = converter.extractAuthentication(payload).getOAuth2Request();
      // Pass back to spring to show audit trail. Shows proof.
